...
app.post('/leads', async (req, res) => {
    try {
        ...
        if (!name) {
            return res.status(400).json({error: "Invalid input: Please add a valid name."})
        }

        if (!source) {
            return res.status(400).json({error: "Invalid input: Please add a valid source"})
        }

        const allowedStatus = ['New', 'Contacted', 'Qualified', 'Proposal Sent', 'Closed']

        if (status && !allowedStatus.includes(status)) {
            return res.status(400).json({error: "Invalid status input: Allowed values are: New, Contacted, Qualified, Proposal Sent, Closed."})
        }

        if (timeToClose && timeToClose <= 0) {
            return res.status(400).json({error: "Invalid input: Please add a positive value for time to close input."})
        }

        const allowedPriority = ['High', 'Medium', 'Low']

        if (priority && !allowedPriority.includes(priority)) {
            return res.status(400).json({error: "Invalid input: Allowed values are: High, Medium and Low."})
        }
        ...
        if (salesAgent) {
          ...
          if (!salesAgentExist) {
            return res.status(400).json({error: `Sales Agent with ${salesAgent} not found.`})
          }
        }
        ...
        res.status(500).json({success: false, message: "Failed to add the lead.", error: error.message})
    }
})
...
app.get("/leads", async (req, res) => {
    try {
        ...
        const allowedSource = ['Website', 'Referral', 'Cold Call', 'Advertisement', 'Email', 'Other']
        ...
        res.status(500).json({message: "Failed to fetch the leads", error: error.message})
    }
})
...
app.patch("/leads/:leadId", async(req, res) => {
    try {
        ...
        const updatedLead = await Lead.findByIdAndUpdate(leadId, updateQuery, {new: true});
...
app.post('/agents', async (req, res) => {
    try {
        ...
        res.status(500).json({message: "Failed to add the agent",error: error.message})
    }
})
